  ...
  if (order == 1 && sigs.BusRd == 1)
    {
      ...
      int n = write(new_socket, &sigs, sizeof(bus_info));
      printf("Written %d and sending %ld\n", n, sigs.uffd);
    }
    ...
    {
      printf("Mch #2 needs to update the bus client_fd %d\n", client_fd);
      int n = recv(client_fd, &recv_signal, sizeof(bus_info), MSG_WAITALL);
      //read(client_fd, &recv_signal, sizeof(bus_info));
      printf("Received: %ld\n", recv_signal.uffd);
    }
...
void* fault_handler_thread(void *arg)
{
  ...
  for (;;) {
    ...
    if (nready == -1)
      errExit("poll");
    update_bus(pollfd.fd, bus_signal);
    nread = read(recv_signal.uffd, &msg, sizeof(msg));
  ...
  if( (new_socket = accept(server_fd, (struct sockaddr *)&cli_addr, (socklen_t*)&client_size)) )
      ...
      if (order == 1)
	{
	  ...
	  do  {	    
		...
		if (accessvar < data.pg_size || accessvar == -1)
		  {
		    if(states_list[accessvar].M == true)
		      {
			printf("Here with %ld\n", accessvar);
		    ...
		    else if(states_list[accessvar].S == true || states_list[accessvar].I == true)
		      {
			bus_signal.BusRdX = true;
		      }
		    else if (accessvar == -1)
		      {
			while (l < data.length)
  ...
  if (order == 2)
    {
      /*MSI Array*/
      unsigned long len;  /* Length of region handled by userfaultfd */
      printf("\nInvoking userfaultfd initialization with client_fd %d\n", client_fd);
	...
	if(commands == 'r')
	  {
		...
		int k = 0x0;
		char c;
		printf("Reading page %ld S: %d M: %d\n", accessvar, states_list[accessvar].S, states_list[accessvar].M);
		if(states_list[accessvar].S == true || states_list[accessvar].M == true)
		  {
		    printf("Check local copy\n");
		    l += (accessvar*page_size);
